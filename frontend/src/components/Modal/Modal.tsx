...
export default function Modal({ isOpen, onClose, data, children }: ModalProps): React.ReactElement {
  return (
    <Dialog open={isOpen} as="div" onClose={onClose} className="relative z-10 focus:outline-none rounded-4xl">
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto bg-black/20 backdrop-blur-xs">
        <DialogPanel
          transition
          className="w-full max-w-md rounded-xl bg-white p-6 backdrop-blur-2xl duration-300 ease-outdata-closed:transform-[scale(95%)] data-closed:opacity-0"
        >
